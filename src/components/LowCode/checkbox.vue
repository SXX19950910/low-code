<template>
  <div v-if="data.parentType !== 'form'" class="native-field-warp" :style="styles">
    ...
    <el-checkbox-group v-else v-model="value" :class="data.field.props.className" @change="onChange">
      <el-checkbox v-for="item in options" :key="item.label" :label="item.id">{{ item.name }}</el-checkbox>
    </el-checkbox-group>
  </div>
