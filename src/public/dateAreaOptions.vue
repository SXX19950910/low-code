<template>
  <el-form class="date-options-warp" label-position="top">
    <div class="title-top">
      当前选中：{{ current.type }}
    </div>
    <el-collapse>
      <el-collapse-item title="基础">
        ...
        <el-form-item label="开始日期占位文本">
          <el-input v-model="current.field.props.startPlaceholder" />
        </el-form-item>
        <el-form-item label="结束日期占位文本">
          <el-input v-model="current.field.props.endPlaceholder" />
        </el-form-item>
        <el-form-item label="范围分隔符">
          <el-input v-model="current.field.props.rangeSeparator" />
        </el-form-item>
        <el-form-item label="快捷选项">
          <el-select v-model="current.field.quick" class="w-100" multiple>
            <el-option v-for="item in quickBasicOptions" :key="item.value" :value="item.value" :label="item.label" />
          </el-select>
        </el-form-item>
      </el-collapse-item>
      <el-collapse-item title="常用">
        <div class="handle-table">
          ...
          <div class="body-sub">
            ...
            <div class="item">
              <div class="name">文本可输入</div>
              <el-checkbox v-model="current.field.props.editable" />
            </div>
            <div class="item">
            ...
            <div class="item">
              <div class="name">取消日期联动</div>
              <el-checkbox v-model="current.field.props.unlinkPanels	" />
            </div>
          </div>
        </div>
      </el-collapse-item>
...
export default {
  ...
  methods: {
    ...
    init() {
      this.form = _.cloneDeep(this.field)
    }
  }
}
</script>
