      ...
      <el-collapse-item title="基础">
        ...
        <el-form-item label="列设置">
          <div class="column-area">
            <draggable v-model="current.field.children" :options="dragOptions">
...
export default {
  ...
  methods: {
    ...mapMutations({
      addGridToFormItem: 'ADD_FORM_ITEM_TO_CHILDREN',
      deleteGridFormItem: 'DELETE_FORM_ITEM_TO_CHILDREN'
    }),
    handleAddFormItem() {
      const grid = {
        children: [],
        elementId: generateId(),
        parentId: this.current.elementId,
        span: 12
      }
      this.addGridToFormItem(grid)
    },
    init() {
      this.form = _.cloneDeep(this.field)
    },
    handleDeleteCol(grid) {
      this.deleteGridFormItem(grid)
    }
  }
}
</script>
